...
When detection returns `kind: "markdown"`, `FileViewer` SHALL render the internal `MarkdownRenderer` with the buffered blob. The `MarkdownRenderer` component MUST NOT be exported as public runtime API.

#### Scenario: Ready markdown file
...
When `api.file.kind` is `"markdown"`, `FileViewerChromeApi` SHALL narrow to `MarkdownChromeApi` with `file: { kind: "markdown"; mimeType: string; downloadUrl: string | null }`. v1 MUST NOT require page, zoom, or search controls on the markdown chrome branch.
