...
`PdfRenderer` SHALL report the page number with the highest intersection ratio in the viewport to the parent via `onVisiblePageChange` when the user scrolls, except during programmatic scroll-to-page (guard ~800ms after `scrollIntoView`).
...
- **AND** the renderer scrolls page 5 into view with `scrollIntoView`
...
`FileViewer` default chrome SHALL provide prev/next, zoom controls, and a page number input for PDFs. `setPage` SHALL update `pdf.page` and `PdfRenderer` SHALL scroll the matching `[data-page-num]` slot into view.
...
- **THEN** page 4's slot SHALL be scrolled into view in the PDF scroll container
...
- **THEN** the renderer SHALL scroll the new page slot into view
