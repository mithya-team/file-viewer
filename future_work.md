...
- Demo fixtures are already present in `apps/demo/public/sample-files` for `txt`, `md`, `html`, `csv`, `jpg`, `pdf`, `docx`, `dotx`, `xlsx`, `pptx` (sample-4, sample-5).
...
- HTML CSP injection / remote-subresource opt-out
- `application/xhtml+xml` support
- additional HTML iframe sandbox tokens beyond `allow-scripts`
